...
export enum EStatusTypesCodeType {
  idle = 'idle',
  get = 'get',
  getOk = 'getOk',
  error = 'error',
...
const initialState: TypesCodeTypeState = {
  ...
  data: undefined,
  isLoading: false,
  status: EStatusTypesCodeType.idle,
};

export const typesCodeTypeReducer = createReducer(
  initialState,
  on(_actions.get, (_state) => ({ ..._state, isLoading: true, status: EStatusTypesCodeType.get })),
  on(_actions.getOk, (_state, pagination) => ({
    ..._state,
    ...pagination,
    isLoading: false,
    status: EStatusTypesCodeType.getOk,
  })),

  on(_actions.error, (_state) => ({ ..._state, isLoading: false, status: EStatusTypesCodeType.error })),
);
...
export class TypesCodeTypeFacade {
  ...
  getAttributeByKeyInArray = (list: any[], value: string, key: string, attribute: string) =>
    list.length && !!value && list.filter((item) => item[key] === value)[0][attribute];
}
