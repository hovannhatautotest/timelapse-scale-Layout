...
export enum EStatusCodeType {
  ...
  get = 'get',
  getOk = 'getOk',
  getListGender = 'getListGender',
  getListGenderOK = 'getListGenderOK',
  error = 'error',
}
export interface CodeTypeState {
  isLoading: boolean;
  listGender: Gender[];
  status: EStatusCodeType;
}

const initialState: CodeTypeState = {
  listGender: [],
  isLoading: false,
  status: EStatusCodeType.idle,
};

export const codeTypeReducer = createReducer(
  initialState,

  on(_actions.getListGender, (_state) => ({ ..._state, isLoading: true, status: EStatusCodeType.getListGender })),
  on(_actions.getListGenderOk, (_state, listGender) => ({
    ..._state,
    ...listGender,
    isLoading: false,
    status: EStatusCodeType.getListGenderOK,
  })),

  on(_actions.error, (_state) => ({ ..._state, isLoading: false, status: EStatusCodeType.error })),
);
