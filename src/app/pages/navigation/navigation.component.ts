...
import { EStatusNavigation, GlobalFacade, Navigation, NavigationFacade, Role, RoleFacade } from '@store';
...
export class NavigationComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.roleFacade.get({ page: 1, size: 30, filter: JSON.stringify({ exceptListRole: 'PARTNER,BROKER' }) });
    this.roleFacade.roles$.pipe(takeUntil(this.destroyed$)).subscribe((list) => {
      if (list && list.length > 0) this.originRoles = list;
    });
    this.navigationFacade.status$.subscribe((value) => {
      switch (value) {
        case EStatusNavigation.postOk:
        case EStatusNavigation.putOk:
          this.isShowForm = false;
          this.navigationFacade.getTree(this.type);
          this.roleFacade.get({ page: 1, size: 30, filter: JSON.stringify({ exceptListRole: 'PARTNER,BROKER' }) });
          break;
        case EStatusNavigation.deleteOk:
          this.navigationFacade.getTree(this.type);
          this.roleFacade.get({ page: 1, size: 30, filter: JSON.stringify({ exceptListRole: 'PARTNER,BROKER' }) });
          break;
      }
