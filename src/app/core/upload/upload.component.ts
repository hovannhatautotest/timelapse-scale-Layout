...
export class UploadComponent {
  ...
  @Input() typeUpload: ETypeFile = ETypeFile.image;
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (response.body?.code === 200) {
            ...
            if (!this.multiple) {
              this.url = response.body.data.physicalPath;
            } else if (typeof this.url === 'object') {
              if (!this.url) this.url = [];
              this.url.push({ file: response.body.data.physicalPath, newPhoto: true });
            }
