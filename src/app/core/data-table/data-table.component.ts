...
export class DataTableComponent implements OnInit, OnChanges {
  ...
  @Input() heightRow = 40;
  @Input() classRow = (data: any, extendData: any): string => data + extendData;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (!this.width && this.columns?.length) {
      let width = 0;
      this.columns.forEach((item) => {
        width += item.tableItem?.width ? parseInt(item.tableItem?.width?.match(/\d+/)![0]) : 200;
      });
      this.width = width + 'px';
    }
  }
