...
import { ETypeFile, FormItemRule, FormModel } from '@model';
...
export class FormComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes?.columns?.currentValue && changes.columns.currentValue.length > 0) {
      ...
      this.columns?.map((item: any) => {
        if (!!item.formItem) {
                ...
                col[subCol.name] = [
                  {
                    value,
                    disabled:
                      subCol.formItem.condition !== undefined
                        ? subCol.formItem.condition(this.values)
                        : !(subCol.formItem?.show === undefined || subCol.formItem.show),
                  },
                  Validators.compose(validators),
                ];
                if (subCol.formItem.condition) subCol.formItem.show = subCol.formItem.condition(this.values);
                return subCol;
              });
          ...
          if (item.formItem.type !== 'tab' && item.formItem.type !== 'addable') {
            ...
            this.arrayKey?.push(item.name);
            if (item.formItem.condition) item.formItem.show = item.formItem.condition(this.values);
                ...
                configForm[item.name] = [
                  {
                    value,
                    disabled:
                      item.formItem.condition !== undefined
              ...
              configForm[item.name] = [
                {
                  ...
                  disabled:
                    item.formItem.condition !== undefined
                      ? item.formItem.condition(this.values)
                      : !(item.formItem.show === undefined || item.formItem.show),
                },
                Validators.compose(validators),
              ];
      ...
      this.onChangeValue(this.$columns[0], '');
    }
              ...
              switch (tab?.formItem?.type) {
                ...
                case 'addable':
                  if (this.values[tab?.name].length) {
                    this.arrayAddable[tab.name] = this.arrayAddable[tab.name].filter((item: any) => {
                      let check = false;
                      for (const [, value] of Object.entries(item)) if (value) check = true;
                      return check;
                    });
                    if (this.arrayAddable[tab.name].length === 0) {
                      const control = <FormArray>this.validateForm.get(tab?.name);
    ...
    if (target !== undefined) {
      ...
      this.$columns?.map((item: any, indexItem) => {
        if (item.formItem && item.formItem.condition && this.$columns) {
          ...
          if (this.$columns[indexItem].formItem?.hasOwnProperty('show'))
        ...
        if (item.formItem?.tab && item.formItem?.columns?.length) {
          item.formItem.columns.map((tabItem: any) => {
            if (tabItem.formItem && tabItem.formItem.condition) {
              ...
              if (tabItem.formItem.hasOwnProperty('show')) tabItem.formItem.show = statusCondition;
            }
          });
        }
      });
  ...
  addField(
    ...
    if (array && columns && name) {
      ...
      columns.map((item: any) => {
        const validators: any[] = this.generatorValidation(item.formItem?.rules, item.formItem.type);
        if (this.arrayKey.indexOf(name) === -1) this.arrayKey?.push(name);
        if (!this.values) {
  ...
  protected readonly ETypeFile = ETypeFile;
}
